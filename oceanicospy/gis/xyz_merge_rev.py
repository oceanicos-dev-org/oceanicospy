...
class XYZFileData:
    ...
    def compute_coverage_polygon(
        ...
        coverage_method: str = "buffer_union",
        target_crs: str = "EPSG:32617",
        buffer_factor: float = 0.8,
        closing_factor: Optional[float] = 1.2,
        simplify_tolerance: Optional[float] = None
    ) -> None:
        """
        Build a detailed coverage polygon from XY points.

        Parameters
        ----------
        coverage_method : {'buffer_union','convex','alpha'}
            - 'buffer_union' (default): union of small buffers around points (concave-like).
            - 'convex': classic convex hull (fast, coarse).
            - 'alpha': concave via alpha-shape (requires SciPy triangulation; optional below).
        target_crs : str
            Projected CRS used for geometric ops (meters).
        buffer_factor : float
            Multiplier for buffer radius = buffer_factor * spacing_estimate.
        closing_factor : float or None
            If given, applies morphological closing: buffer(+r2) then buffer(-r2),
            where r2 = closing_factor * buffer_radius.
        simplify_tolerance : float or None
            Optional Douglas–Peucker simplification in meters (after union/closing).
        """
        # 1) Prepare GeoDataFrame in WGS84
        df = self.points.copy()
        ...
        # 2) Project to a metric CRS for geometry ops
        gdf = gdf_wgs.to_crs(target_crs)

        if coverage_method == "convex":
            polygon = gdf.unary_union.convex_hull

        elif coverage_method == "buffer_union":
            # 3) Radius from spacing (guards against gaps/holes inside the tile)
            r = max(0.1, float(self.spacing_estimate) * buffer_factor)

            # 4) Union of buffers
            buff = gdf.buffer(r)
            merged = unary_union(buff)

            # 5) Optional morphological closing (smooths small voids)
            if closing_factor is not None:
                r2 = r * float(closing_factor)
                merged = merged.buffer(r2).buffer(-r2)

            polygon = merged.convex_hull if merged.geom_type == "GeometryCollection" else merged

            # 6) Optional simplification
            if simplify_tolerance is not None and simplify_tolerance > 0:
                polygon = polygon.simplify(simplify_tolerance, preserve_topology=True)

            # Keep only outer envelope if multipolygon but all parts touch
            if polygon.geom_type == "MultiPolygon":
                polygon = max(list(polygon.geoms), key=lambda p: p.area)

        elif coverage_method == "alpha":
            # --- Optional alpha-shape without extra deps (SciPy required) ---
            from shapely.ops import triangulate
            import numpy as np

            pts = np.array([[pt.x, pt.y] for pt in gdf.geometry])
            tri_list = triangulate(gdf.unary_union)
            # Characteristic length from spacing
            L = float(self.spacing_estimate)
            # Filter triangles by a compactness criterion ~ alpha
            alpha = 2.5 * L  # tune 1.5–3.5
            kept = []
            for tri in tri_list:
                # Keep triangles whose circumradius is below alpha
                a, b, c = tri.exterior.coords[:3]
                la = np.hypot(a[0]-b[0], a[1]-b[1])
                lb = np.hypot(b[0]-c[0], b[1]-c[1])
                lc = np.hypot(c[0]-a[0], c[1]-a[1])
                s = 0.5*(la+lb+lc)
                area = max(tri.area, 1e-12)
                R = (la*lb*lc)/(4.0*area)  # circumradius
                if R <= alpha:
                    kept.append(tri)
            if not kept:
                polygon = gdf.unary_union.convex_hull
            else:
                polygon = unary_union(kept)
                if polygon.geom_type == "MultiPolygon":
                    polygon = max(list(polygon.geoms), key=lambda p: p.area)
        else:
            raise ValueError("Unknown coverage_method")

        # 7) Back to WGS84 for storage/plotting
        poly_wgs = gpd.GeoSeries([polygon], crs=target_crs).to_crs("EPSG:4326")
        self.coverage_polygon = gpd.GeoDataFrame({"geometry": poly_wgs}, crs="EPSG:4326")
...
class XYZMerger:
    ...
    def _merge_tiles_respecting_priority_with_polygons(self, target_crs: str = "EPSG:32617") -> None:
        """
        Merge tiles by removing points from lower-priority tiles that fall within any
        higher-priority coverage polygon. Geometry operations are performed in a
        projected CRS (meters) for accurate spatial predicates. The final merged
        DataFrame is stored in self._merged_df.
        """
        # Ensure tiles are ordered by ascending priority_rank
        self._tiles.sort(key=lambda t: t.priority_rank)

        # Build GeoDataFrames for points and project both points and polygons
        tile_points_gdf_list: List[gpd.GeoDataFrame] = []
        tile_polygons_list: List[gpd.GeoDataFrame] = []

        for tile in self._tiles:
            # Points start in WGS84 as numeric columns x,y; geometry is created and projected
            ...
            # Each tile already has coverage_polygon in WGS84; project it too
            poly_proj = tile.coverage_polygon.to_crs(target_crs)

            tile_points_gdf_list.append(gdf_points)
            tile_polygons_list.append(poly_proj)
        ...
        for i, _ in enumerate(self._tiles):
            ...
            # Concatenate polygons from all higher-priority tiles (already in target_crs)
            ...
            # Spatial join in projected CRS for accurate 'within' test
            joined = gpd.sjoin(
                gdf_points_i,
            ...
            # Keep only points NOT within any higher-priority polygon

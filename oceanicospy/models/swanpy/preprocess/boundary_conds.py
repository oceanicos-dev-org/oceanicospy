...
class BoundaryConditions():
    ...
    def _process_boundary_points(self,points_lat,points_lon,single_tpar_fn):
        ...
        for idx_lon,lon in enumerate(points_lon):
            single_tpar_fn(f'{self.input_path}TparN2025_{idx_lon+1}',max(points_lat),lon)
            single_tpar_fn(f'{self.input_path}TparS2025_{idx_lon+1}',min(points_lat),lon)

        for idx_lat,lat in enumerate(points_lat):
            single_tpar_fn(f'{self.input_path}TparE2025_{idx_lat+1}',lat,max(points_lon))
            single_tpar_fn(f'{self.input_path}TparO2025_{idx_lat+1}',lat,min(points_lon))
        ...
        if self.dict_info != None:
            ...
            if self.boundary_type == 'side':
                ...
                for side in self.list_sides:
                    if side not in ['N','S','E','O']:
                        raise ValueError(f"Invalid boundary side '{side}' provided. Valid options are 'N', 'S', 'E', 'O'.")
                    else:
                        self.boundary_line = f"BOUN SIDE {side} CLOCKW CON PAR {wave_params['hs']} {wave_params['tp']} {wave_params['dir']} {wave_params['spread']}"
            else:
                pass
        else:
            raise ValueError("Boundary type information is missing in the provided dictionary. Please ensure 'boundary_type' key is included.")
